...
class ModelLaplace2
{
    static void Main(string[] args)
    {
        ...
        VariableArray<double> noise1 = Variable.Array<double>(grid_x);
        noise1[grid_x] = Variable.GaussianFromMeanAndPrecision(0.01, 100).ForEach(grid_x);
        VariableArray<double> noise2 = Variable.Array<double>(grid_x);
        noise2[grid_x] = Variable.GaussianFromMeanAndPrecision(0.01, 100).ForEach(grid_x);
        ...
        using (ForEachBlock t_block = Variable.ForEach(grid_tt))
        {
            ...
            using (ForEachBlock x_block = Variable.ForEach(grid_xx))
            {
                ...
                using (Variable.If(t > 0))
                {
                    ...
                    using (Variable.If(x > 0)) 
                    {
                        Variable.ConstrainEqual(solution[t-1][x] + solution[t + 1][x] - 4 * solution[t][x] + solution[t][x - 1] + solution[t][x + 1] - function_mean[x] * function_t[t] * dx * dx, 0);
                    }
                    Variable.ConstrainEqual(solution[t][size_x], 0);
                }
            }
        ...
        using (ForEachBlock x_block = Variable.ForEach(grid_xx))
        {
            ...
            using (Variable.If(x > 0))
            {
                //Variable.ConstrainBetween(noise1[x], 0, 3 * 1e-4);
                //Variable.ConstrainBetween(noise2[x], 0, 0.0003);
                ...
                Variable.ConstrainEqual(2 * solution[1][x] + solution[0][x - 1] + solution[0][x + 1] - 4 * solution[0][x] - 2 * dx * (derivative_start_function[x]) - function_mean[x] * function_t[0] * dx, 0);
                Variable.ConstrainEqual(2 * solution[size_x - 1][x] + solution[size_x][x - 1] + solution[size_x][x + 1] - 4 * solution[size_x][x] - 2 * dx * (derivative_end_function[x]) - function_mean[x] * function_t[size_x] * dx, 0);
            }
        }
        ...
        engine.NumberOfIterations = 275;
        //Console.WriteLine(engine.Infer(solution));
        var prediction_loc = engine.Infer<DistributionStructArray<Gaussian, double>>(function_mean);
        //Console.WriteLine(engine.Infer(function_prec));
